...
case $1 in
    location)
        location=$(echo -e "Strasbourg\nAltkirch\nActuel" | rofi -dmenu -i -mesg "Sélectionner une localisation")
        if [ -n "$location" ]; then
            if [ "$location" == "Actuel" ]; then
                location=""
            fi
            rm -f "$cache"
            curl -s --fail --max-time 10 -A "waybar-weather" -H "Accept-Language: fr" "https://wttr.in/$location?format=%C+%t" > "$cache"
        fi
        ;;
    *)
        exec 200>"$lock"
